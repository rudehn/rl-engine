...
- Cues: `Cued`, `Cue`, `Anchor`, `LookOf` and `TurnHold` in `rl-bevy`. The resolvers write what is worth seeing as anchors that follow the entities they are on, an effect writes its own through `EffectWorld::cues`, and the turn loop waits while a plugin that watches plays them, never for a cue that lands on the player. `ParticlesPlugin` plays cues, one animation per actor in the order written, a trail bending to where its target is now, and watches the hold unless `ParticleStyle::instant`; a key pressed while the turns wait skips what holds them and is read with the turn in hand. `AbilityEvent::Used` and `ItemEvent::Thrown` no longer carry a path; the cues do.
