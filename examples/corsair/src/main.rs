...
use rl_engine::rl_ui::{
    AbilityPanel, Facets, GearPanel, InspectPanel, LogPanel, MessageLog, Modals, NearbyPanel, NearbyView, ScrollbackPanel, TargetPanel, Tones, UiPlugin,
...
fn main() -> AppExit {
    ...
    .add_plugins((CorePlugin, FovPlugin, CombatPlugin, StatusPlugin, ItemsPlugin, LightingPlugin, StreamingPlugin, FactsPlugin, AbilitiesPlugin))
...
fn spawn_fresh_player(world: &mut World, spawn: rl_engine::rl_core::Point) {
    ...
    world.spawn((
        ...
        (Known::new(), grants, Cooldowns::new()),
    ));
}
