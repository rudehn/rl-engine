...
use rl_engine::rl_bevy::{EngineState, Restart};
...
impl Plugin for TitlePlugin {
    fn build(&self, app: &mut App) {
        app.init_resource::<Title>()
            // Before the engine's input phase and outside every play-gated
            // set, because the whole point of this screen is the time
            // before a run exists. Its own exclusive system for the pick,
            // since starting the run means running `NewRun`.
            .add_systems(Update, read_title_keys.before(EngineSet::Input).run_if(title_is_up))
            .add_systems(Update, draw_title.after(read_title_keys).run_if(title_is_up));
    }
}
...
///
/// Exclusive, because taking up a run runs the `NewRun` schedule: the same
/// thing the engine does for a restart, from the one place that knows the
/// player asked.
pub fn read_title_keys(world: &mut World) {
    let keys = world.resource::<ButtonInput<KeyCode>>();
    let (up, down) =
        (keys.just_pressed(KeyCode::ArrowUp) || keys.just_pressed(KeyCode::KeyK), keys.just_pressed(KeyCode::ArrowDown) || keys.just_pressed(KeyCode::KeyJ));
    let take = keys.just_pressed(KeyCode::Enter) || keys.just_pressed(KeyCode::NumpadEnter) || keys.just_pressed(KeyCode::Space);
    let quit = keys.just_pressed(KeyCode::Escape) || keys.just_pressed(KeyCode::KeyQ);
    ...
    let picked = Choice::all()[title.picked.min(rows - 1)];
    if quit {
...
/// Lowers the screen and starts the run the player asked for.
///
/// `NewRun` rather than a `Restart`: a restart tears down a run and waits
/// for the state to come round to idle, and there is no run to tear down
/// yet. The message is written all the same, for anything that keeps count
/// of runs.
fn begin(world: &mut World) {
    world.resource_mut::<Title>().up = false;
...
mod rows {
    ...
    /// The first row of the menu.
    pub const MENU: i32 = 33;
}
...
/// Where the welding arms hang, between the towers.
const ARMS: [i32; 2] = [44, 70];
...
fn paint(terminal: &mut Terminal, t: f32) {
    ...
    // Sparks up the chimney: they start at the furnace's crown and go out
    // on the way to the crane's rail, so none of them is ever drawn inside
    // the brick.
    for i in 0..10 {
        let drift = ((t * 5.0 + i as f32 * 2.3) % 9.0).floor() as i32;
        let x = FURNACE_X + 4 + (hashed(i, 7) * 13.0) as i32;
        let y = rows::TOWER - 1 - drift;
        if y > rows::CRANE {
            terminal.set(x, y, Cell::new(if drift < 2 { '*' } else { '.' }, ink::SPARK));
        }
    }
    ...
    // A hanger for each arm, up to the rail, so they are held by something.
    for x in ARMS {
        for y in rows::CRANE + 1..rows::ARM {
    ...
    for x in (8..w - 8).step_by(17) {
        ...
        for dx in -1..=1 {
            terminal.set(x + dx, rows::LAMP + 2, Cell::new('·', ink::LAMP.with_alpha(0.3)));
        }
    }
}
...
fn paint_title(terminal: &mut Terminal, t: f32) {
    ...
    let y = rows::TITLE;
    // Lit from the works below: a shadow down and to the right in the deep
    // ember the furnace throws, laid in first so the letters sit on it.
    let mut shadow = x + 1;
    for letter in &letters {
        for (row, line) in letter.iter().enumerate() {
            let glyphs: String = line.chars().map(|c| if c == ' ' { ' ' } else { '▒' }).collect();
            stamp(terminal, shadow, y + row as i32 + 1, &[glyphs.as_str()], ink::EMBER_DEEP.with_alpha(0.45));
        }
        shadow += letter[0].chars().count() as i32 + 1;
    }
    for letter in &letters {
...
fn paint_menu(terminal: &mut Terminal, title: &Title, palette: &Palette) {
    ...
    terminal.print(centre(keys), terminal.height() - 2, keys, palette.get(Tones::MUTED));
}
