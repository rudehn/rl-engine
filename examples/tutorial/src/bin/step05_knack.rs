//! Warren, step 5: a knack of your own to call on.
//!
//! The guide chapter is `docs/guide/src/05-a-knack.md`. An ability is a
//! line of RON: an aim, a shape, a cooldown and a list of effects. A key
//! writes [`AimAt`] and the engine does the rest, cursor and all.
//!
//! `cargo run -p tutorial --bin step05_knack`
//!
//! Keys: arrows to walk or strike, `a` screech, `g` get, `e` eat,
//! `r` throw a rock, `t` lantern, `.` wait, `q` quit.
...
fn main() -> AppExit {
    ...
    app.add_plugins(RoguelikePlugins::new("Warren", COLS, ROWS).map(Rect::new(0, 1, COLS, ROWS - 1 - LOG_ROWS)))
        ...
        .add_plugins(VitalsPanel::new(Rect::new(0, 0, COLS, 1)).hints("[a]screech [g]et [e]at [r]ock [t]orch [.]wait [q]uit"))
