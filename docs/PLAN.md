...
- 2026-09-15: particles, and an aim that says when it cannot reach.
  `rl-render` gains `particles`: an `Animation` of `Trail` and `Burst` steps, played from the wall clock over the map after the turn that caused it, since the turn loop is instant and a fireball would otherwise land before it was seen to fly. It never waits: the world has moved on, and a player who acts while a burst fades gets the next turn at once. `ParticlesPlugin` is in `RoguelikePlugins` and plays one for every ability used, in the `look` the ability's RON gives it, and every item thrown, in the item's glyph; the events carry the path for it.
