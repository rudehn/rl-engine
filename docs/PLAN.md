...
- 2026-09-17: weapon speed, and eight facings for a prefab.
  `MeleeAttack` and `RangedAttack` gain `cost`, hundredths of a step that `resolve_attacks` charges instead of the flat `BASE_ACTION_COST`; `None` charges exactly that base cost, so nothing built against the old shape moves, and a shot that finds nothing in reach still spends the ordinary turn it aimed rather than a free one. A game asked for a fast weapon and a slow one to actually differ, which the cost had never let them do.
  `rl-mapgen` gains `Prefab::rotated` and `Prefab::flipped`, which carry a piece's marks with its tiles rather than leaving them behind, `Orient` (`Fixed`, `Turned`, `TurnedOrMirrored`) on `StampPrefab` drawing a facing from the pass's own stream, and `StampOneOf`, one stamp choosing among several weighted candidates and failing the chain loudly when none carries weight. `Fixed` draws nothing from the stream, so every map already built from a `StampPrefab` is unchanged; one authored vault laid down in eight facings is most of what a hand-built dungeon means by variety.
