...
- 2026-09-14: a release has a page, and generating a game names the release.
  `v0.1.0` was a tag with no release page. It has one now, and `.github/workflows/release.yml` publishes the next from `CHANGELOG.md` through `scripts/release-notes.sh` when its tag is pushed, after checking the tag is the workspace version.
  Found on the way: `cargo generate --git ... templates/starter` takes the template from `main`, while the template pins the engine to `v0.1.0`, so the first breaking change on `main` would generate games that do not build. The README, the guide and the release notes pass `--tag v0.1.0`, which takes the template from the release as well, and `check-template.sh` refuses a generate line without it.
- 2026-09-14: minds have wits, and doors open.
  Nothing said what a monster was able to do: a brain with `FleeWhenHurt` in it fled whether it was a jaguar or a slime, and a game that wanted a crab to stay a crab built it a different brain. And no game had a door: Corsair, the delve and the tutorial each registered a walkable opaque tile and called it one, because the engine had no tile that opened.
  `rl-rules` gains `Wits`, a set of capabilities (flees, searches, opens doors) with `MINDLESS`, `ANIMAL` and `SAPIENT` presets, written in content as a preset or a list that may mix both. A set rather than a closed ladder of levels, because a level is only a preset and a game's own tier is a combination, not a variant. `Snapshot::wits` carries it and `FleeWhenHurt` and `SearchLastKnown` ask it first, so one brain shape serves a bestiary and the wits say how much each kind uses.
  `rl-grid`'s `TileProps` gains `opens_to` and `closes_to`, resolved by name in `tables()`, which refuses a name nobody registered; the standard doors open and close into each other. `rl-bevy` gains `Intelligence(Wits)`, required by `Mind` and sapient by default so an unflagged monster does all its brain asks, and `doors`: walking into a closed door opens it for an actor with the wits, spending the turn where it stands; `Close(Direction)` shuts one unless something stands or lies in the doorway; `DoorEvent` narrates both. A mind that opens doors reads the map through `WorldMap::opening_view` and paths through a door when that is shorter, and flow fields are keyed by it; `WorldMap::cost_epoch` rebuilds them when a door changes and nobody moved.
  Corsair's doors open and shut, its crab is mindless, its dog and jaguar animals and its cutthroats and marines sapient, `c` shuts a door, and the log says when a monster in sight works one. The delve's beasts carry wits; its sinew curtains stay curtains. The template's goblins say they are sapient and what the other two presets would change.
- 2026-09-14: throwing, putting on from the ground, and minds with hands.
  Deferred since M3. `rl-bevy` gains `throwing`, its own plugin because a throw is both an item leaving a bag and a blow down combat's pipeline, and neither of those plugins should know the other: `Throwable` (a range and an optional strike), the `Throw` action at a cell, one from a stack or the item itself, and `flight`, which stops at the first wall or body the way a bolt does, so a knife and a spell agree about what is in the way. The item rests at the feet of whoever it struck, short of a wall, or at the end of its reach, and `ItemEvent::Thrown` reports it. The targeting cursor aims a throw from `AimThrow` and previews it through the same `flight`.
  `EquipFromGround` costs a turn and a half, cheaper than picking up and putting on in two turns and dearer than either; `GearScore` is what wearing an item is worth, in whatever units the game likes, because only the comparison matters. `Wits` gains picks up, equips and throws; the snapshot holds what a mind carries to throw and what it sees lying about, with the gain of wearing each against what it would displace, worked out on a copy of its equipment; `ThrowAtRange` throws down a clear line at an enemy out of arm's reach, and `Scavenge` fetches better gear or something to throw by a bounded shortest walk and puts gear on where it lies. What a dead actor carried falls where it died, rather than leaving the world with it.
  Found on the way, reproduced in a test before the fix: picking an item up took its position and left its map, so an item carried downstairs and put down still claimed the map it came from, drawn nowhere and impossible to take back up. A carried item is on no map now, and one put down or thrown is on the map it lands on. And `TargetViewPlugin` in a game without abilities failed on its first frame, since the cursor writes `Intent<Use>` and only `AbilitiesPlugin` registered it; the cursor registers both intents it writes.
  Corsair's cutthroats come with throwing knives and throw them, its marines put on the better gear they find on top of their own blow and hide, one brain shape serves the whole bestiary with the wits deciding, the player starts with knives, `r` and the chest's `t` throw and `e` puts on what lies underfoot, the log narrates what a monster in sight takes up, wears or throws, and a monster's bag survives a save.
- 2026-09-15: tile fields, and fire and gas on them.
  Section 3.9 asked for one `TileField<T>` with a rule per kind, double-buffered and allocating nothing in a step, with `fantasy-rogue`'s gas as the reference. `rl-grid` gains it; `docs/design/fields.md` is the design.
  Gas is content: `GasDef` holds only what the engine acts on (spread, fade, the concentration that hides what is behind it, whether it burns, and a status for breathing enough), `gas::load` reads it by status name, and `Registries::gases` holds it. Spreading is an exchange with each neighbour and every cell then fades by at least one unit, so the densest cell never gains and every cloud clears, which a property test over thirty-two clouds of every shape holds. A tile that stops a thrown thing stops gas. `GasPlugin` gives off what `Release` asks and what `Vents` puts out, writes gas thick enough into `WorldMap::set_veil`, which field of view and light both read, and reports `Breathed`.
  Fire is the engine's: `TileProps::burn` gives a chance to catch, a duration and the tile it leaves, and the tile it leaves is required, because a tile that stayed itself could catch again from the neighbour it lit and two would pass one fire between them forever. `fire::spread` rolls one chance per burning neighbour from a hash of the seed, the turn and the cell, so it is order-independent and a save holds no generator. What a game burns carries `Flammable`, a gas that `burns` feeds it and is used up, and `FireRules` says what standing in fire inflicts, what gas it smokes and how it glows; `FireEvent` reports scorching, catching, burning out and burnt ground, and what burnt out is the game's to despawn or char. `Ignite` and `Emit` are ability effects the two plugins register.
  Both keep a field per map, set aside while the player is elsewhere and saved in `EngineSave::fields`, and run in `ResolveSet::Fields`, fire before gas so smoke from a burning vapour spreads the turn it rises, and before the turn's ticks so a status fire puts on bites the turn it lands. The map view draws flames and tints the ground under gas from `FieldAppearance`, and minds will not step into fire.
  Found on the way: a `Burning` fixture lit its cell only once the first turn had passed, so a monster deciding before then walked into the brazier; burning things now keep their cells alight at the start of every pass.
  The delve's fat slicks catch and burn to cinder, its sinew curtains burn away, one bile pool in four reeks of a gas that burns and dazes, burning flesh smokes, and the fireball sets what it lands on alight.
- 2026-09-15: one list of what is in sight, and one thing picked out of it.
  `rl-ui` gains `focus`: `InSight`, actors nearest first and then things, the order the nearby rail prints, and `Focus`, one entity picked out of it. With nothing open, Tab steps the focus down the rail, Shift steps it back and Escape lets go, and `NearbyPanel` draws the row and its map tile on the selection tone, scrolling to keep the row shown.
  Both cursors cycle that list entity by entity, so two things on a tile are two stops, open on the focus when they can take it, and move it as they go. The look cursor stops on things as well as actors. The targeting cursor narrows the list by `Aim::cycles_to`, and an aim at the ground stops on what is in sight rather than on cells, which the direction keys reach anyway. `AimFire` aims a `RangedAttack` through the same cursor, previewed through `shot`, the call `line_of_fire` now answers by; Corsair's `f` uses it instead of shooting the nearest foe unasked.
  Found on the way: the delve's whaler's lamp spawned without `Item`, so `g` on it found nothing and said nothing; it is carried and set down like the torch now, and `g` and `d` say when there is nothing to do.
  Nate, 2026-09-15: Tab should cycle the nearby list's targets in every cursor, and highlight the list outside one.
- 2026-09-15: every key, declared once and listed on a screen.
  `rl-ui` gains `controls`: a game declares each key it answers to as a group, an action and the keys, keeps the `ControlId`, and reads a frame's keys through `ControlInput` by that id. `ControlsPanel` lists the same registry on `?`, grouped and paged, with a hint row naming the key, so the screen cannot show a key the game does not read. The engine's own screens declare theirs from their binding resources, in `finish` so a game's groups are listed first, and a rebound cursor key is listed as rebound. A `Chord` is matched with Shift exactly, which is what let the delve's `L` stop being a step east as well as the brand. Corsair, the delve, the starter and the tutorial's step 10 read their keys by name, and their hand-typed key lines are gone.
  Section 8 of `docs/design/ui.md` had refused generated hints until there was a registry to generate them from; there is one now, and the controls screen is the first thing generated from it.
  Nate, 2026-09-15: a "show controls" hint that lists every key, built into the engine and exposed through each game.
- 2026-09-15: the unload bridge, the last piece deferred from M6.
  `rl-save` gains `Stash` and `UnloadPlugin`. The page's `pagehide` and `beforeunload` handlers run outside the app and cannot borrow the world, so the design from `fantasy-rogue`'s `web_unload.rs` holds: the game encodes its save into the stash whenever it likes, and the handler, leaked on purpose, writes the stash to storage synchronously. The same flush runs on the frame the app exits, so a native window closed by its button saves too. `Saves` shares its backend by `Arc` so the handler writes through the one the game reads. Corsair stashes once a turn and clears the stash on the death that deletes the save, or closing the window afterwards would write the run back.
  Also: Corsair's chest and ledger hints come from the controls registry, and the guide says why steps 2 to 9 keep raw key codes.
- 2026-09-15: the character sheet, deferred since M3.
  `rl-ui` gains `SheetView` and `SheetPanel`. The vitals strip says what the player has; the sheet says why: each registered stat with its base, its final value and every modifier between them, tagged by source, and the source named where the engine can name it (a status tags its own) or where the game does (`name_source`, which Corsair calls for its items in `ViewSet::Annotate`). Then resists, the blows and the shot, the statuses with the turns they have left and what each does, and every slot. All engine state already, which is what made it a view and not a game's screen. The panel is a modal on `SheetKeys`, listed on `?`; Corsair puts it on `@` since `c` shuts doors there, and a Shift and digit chord now labels as the symbol it types.
- 2026-09-15: the ability menu, data-driven and the engine's end to end.
  The list on `a` could be opened and closed and nothing else: no way to walk it, nothing said about a knack, and a knack pressed over it was swallowed. Now `AbilityPanel` runs the menu itself, on the keys every other screen uses, and describes the row picked out from the ability's own data: `AbilityDef::description` (written in RON), the aim and shape in one phrase, costs and requirements with the registries' names, time and cooldown, and each effect through a new `Effect::describe`, so an ability added in RON gets its entry for free and an effect a game adds writes one method to be described. Confirming a row writes `AimAt`, the same message a hotkey writes.
  Found on the way, and the likely cause of a fireball that "did nothing": the Enter that confirms the cursor closed it and was then read by the game's input the same frame, as the stairs; the stairs resolve before abilities and, refused, still marked the player as acted, so the use was dropped. Which of the two ran first was the scheduler's choice. `Modals::any_open` now stays true for the rest of the frame a screen closes on, and the cursor is not steered on the frame it opens, so the Enter that aims from the menu cannot confirm the aim it opened. Both are played through the real keys in the delve's tests.
- 2026-09-15: particles, and an aim that says when it cannot reach.
  `rl-render` gains `particles`: an `Animation` of `Trail` and `Burst` steps, played from the wall clock over the map after the turn that caused it, since the turn loop is instant and a fireball would otherwise land before it was seen to fly. `ParticlesPlugin` is in `RoguelikePlugins` and plays one for every ability used, in the `look` the ability's RON gives it, and every item thrown, in the item's glyph.
- 2026-09-15: a projectile lands after it is seen to fly.
  A capture into the hold showed the fire down and the eels dead while the fireball was still in the air, because the world is instant and only the drawing lagged. Hiding the world's change until the burst would have meant a second, stale world for the map, the log and the panels to draw from; instead the landing is put off. With something watching, the ability and throw resolvers cue the flight, spend the turn, and put the landing aside; `schedule` deals no turn while anything is in the air; and the first pass after the hold lets go lands it and cues the burst, which the loop then waits on in turn. So the order on screen is the order in the world: flight, landing, fire, and only then the next actor. Headless, nothing watches and a use lands as it is cast, so every existing test holds. Nate, 2026-09-15: "projectile, then it lands, then it triggers the effect like fire."
- 2026-09-15: seed replay, the last piece deferred from M6.
  A run is its seed and the player's keys, now that the loop, the minds and every roll are deterministic given the seed, so that is all a `Recording` holds: RON, small enough to paste into a bug report. Each key carries the turn clock it was read at rather than a frame number, since frames come at whatever pace the machine and the animations set and the clock does not; a replay presses a key when the player holds a turn, nothing is held for a cue, and the clock reads what it read then, and a clock the game has passed is a drift, reported by key number and stopped at, since pressing on would bury where the difference began. The held-key repeat is written as the presses it stood for, so the wall clock is nowhere in the file. `replay::args` and `replay::seed` answer from the recording when one is named, and both games read their flags through them, so a replay runs on the floor and the world it was recorded in. Found on the way: a capture could only ever photograph what an animation left behind, so `RL_CAPTURE_AT=hold` shoots into the hold instead.
- 2026-09-15: cues, and turns that wait to be seen.
  Particles played from the events and never waited, so a monster's spit and the crab's step after it landed in one frame, and a burst on someone knocked back stayed on the floor they left. The resolvers now write `Cued` messages in `rl-bevy`: a flight between two `Anchor`s or a burst on some, where an anchor is a cell or an entity followed as it moves, so a bolt at a player who walks on lands on the player and a pulse on someone who steps aside goes with them. The look is a `LookOf` the drawer resolves, so tier 2 says nothing about glyphs. `TurnHold` is the engine's wait: a plugin that plays cues watches it, the loop stops after any pass that cued something and runs no pass until the plugin lets go, and a cue at the player holds like any other, so a spit is seen to arrive before it hurts; what keeps the player from waiting on it is the key: pressed while the turns wait, it skips through every hold in the way to the player's own turn, and once the player holds a turn nothing holds the turns, since there is nothing left to wait for. Nate, 2026-09-15, on the earlier exemption: letting the flight land on arrival and the key skip it "sounds good". One animation per actor per frame, in the order written, is what makes a drain read as out and back: the resolver cues the flight and the burst before its effects run, and the game's `Drain` cues the return after. A key pressed while the turns wait skips what holds them and runs the turns on before the game reads the key, since a player who casts and reaches for the next key must not lose it to the fade; the turn loop runs from that exclusive system as it does from its own set. A headless game inserts `ParticleStyle::instant` and is never held, since even a frame's wait would put the player's turn past the frame a scripted key is read in.
  An aim past where its shape reaches was landed where the projectile stopped, which is a fireball bursting on a spot nobody chose, and the overlay gave no sign. It is refused now, `Blocked::OutOfReach`, and the overlay paints the rest of the line in the bad tone with a pulse running towards the cursor, so the line reads as a direction with a limit rather than a wall. The shot cursor reads the same way.
  Found on the way: both games narrated an ability after the blows it landed, because their narration systems were a tuple and the scheduler chose; they are chained now.
  Nate, 2026-09-15: the log order, a spell seen to travel and detonate, particle support in the engine, and a red line past the range with a pulse towards the target.
- 2026-09-15: the stat fold, the first item of the architecture review's `docs/TODO.md`.
  Combat read `Armor`, `MeleeAttack`, `RangedAttack` and `Strikes` off the actor while `StatBlock` held the registered stats, and nothing joined the two: Corsair's `refresh_gear` rebuilt all four from worn items and affixes on every `Changed<Equipped>`, kept an `Innate` to remember what a marine had before it dressed, and inserted and removed a `RangedAttack` by hand, fifty-five lines every game with gear would have written and got a detail of wrong.
  Now an item says what wearing it does with the components an actor already has, enchant folded in at spawn since an enchant is rolled per instance, plus `Bestows` for what it changes on a stat; `rl-bevy` gains `Loadout`, the one answer to what an actor fights with, summed at the blow from the actor's own components, its worn items' in slot order, and the stats `CombatRules` names with `armor_stat` and `attack_stat`; the attack resolver, `apply_damage`, the inspect forecast, the vitals strip and the sheet all read it, the way they read `Bystanders::land`. `fold_gear` in the items plugin puts every worn item's `Bestows` onto the wearer under `Source::Item` and strips them with it, and since it runs on `Changed<Equipped>` a wearer spawned dressed is folded on its first pass, so a restored run rebuilds its gear modifiers from its slots and the save never writes them down.
  `Modifier::source` became a `Source` enum rather than a raw `u64`, because two systems fold modifiers into one `Stats` without knowing about each other and the status test was "anything that is not a status", which a third kind of source would have broken. The sheet names a worn item's changes after the item's `Name`, which took Corsair's annotate system for that with it. Corsair's `refresh_gear` and `Innate` are gone and its marine test asserts through `Loadout` that nothing on the marine changed when it dressed.
  Decisions, Nate, 2026-09-15: health stays a component, since hit points are state and a maximum-health modifier is separate work; and the fold rebuilds from `Equipped` on restore rather than the save carrying modifiers tagged with entity bits that would not survive it.
- 2026-09-15: the inventory screen, the second item of `docs/TODO.md`.
  Corsair's sea chest was 201 lines over `Inventory`, `Equipped` and `Name`, with the rows' numbers read back out of the armory's definitions, and every game with items would have written the same screen.
  `rl-ui` gains `InventoryView`, the bag in pickup order with each row's worth read off the item's own components, the ones `Loadout` reads, so an item spawned with a `MeleeAttack` or `Bestows` is described for free in the registries' names and a game says nothing twice; and `InventoryPanel`, the screen run end to end the way the ability menu is: `InventoryKeys` opens it, the direction keys walk it, wear, drop and use write the engine's own item intents for the row picked out, throw opens the targeting cursor through `AimThrow`, and any action closes every screen since it spends the turn. What using an item means stays the game's, answered from `ItemEvent::Used`, and what it does is a facet the game pushes: Corsair's "restores 8 health when drunk" is one.
  The bag's keys are engine keys, so the controls screen lists them under the panel's title and a game that rebinds one sees the new key there. Corsair's `inventory` module is gone and its chest keys with it; the delve gains a pack it never had.
- 2026-09-15: the narrator, alternate actions, and the run's beginning and end, from `docs/TODO.md` and Nate's asks of the same day.
  Ten files turned `DamageDealt` and `DeathEvent` into the same log lines, each branching on the player and each getting the order wrong across a frame of many turns; nine input systems held the same match on `Occupancy::first_at`; and every game ended a run by flipping `EngineState::Idle` and telling the player to press `q`.
  `rl-bevy` gains `ResolveSet::Redirect` and `Bump`, the action that becomes another: a step, an `Open` or an `Attack` by what is in the way, written in `Redirect` and claimed by whichever resolver owns what it became. Doors are their own actions now, `Open` beside `Close`, because working one changes what blocks sight; a step into a shut door is refused and a mind writes `Open` when its path reaches one. A bump into someone who is not a foe is a free refusal and a `Bumped` for the narrator; without rules to call anyone a foe, nobody is one.
  `rl-ui` gains the narrator: `NarrationView` collected in `TurnSet::React` one pass at a time, since one pass is one actor's action and that is the only place the true order can be read, and `NarratorPlugin` speaking each row through a `Phrasebook` split by perspective, so there is no grammar in the engine and a game rewords, silences or speaks the unseen. Names wear the colour of the thing they name, as `Span`s on the log line, lifted to readable by the panels; the player's blows and kills are `Tones::HIT` and `Tones::KILL`, brighter than text, and a kill by the player is its own phrase. The games lost their narrators; Corsair keeps its status words and silences three phrases, the tutorial rewords one so a rat bites.
  The run has a life: `NewRun` is where a start system goes, run at startup and after every `Restart`, which runs `EndRun` for what the game keeps, despawns everything on a map, resets what the engine keeps, removes the map so a start that forgets one is told, and comes back through `Idle` so everything that watches for play beginning sees it again. `RunOver` ends a run with an `Outcome` and the game's epitaph, combat writes it on the player's death unless the rules say a death is not the end, and `EngineState::Over` draws the world and runs nothing else. `GameMenuPanel` is the menu on Escape and the screen a run ends on, offering a new run, the same seed or quitting; on the ending it files an `Obituary` in the `Morgue`, `rl-save`'s record of a run through the same backend seam a save uses, with the game's own sections pushed as it reads `RunOver`.
  Nate, 2026-09-15: a YouKill phrase, the player's hits and kills highlighted, names coloured as the things are, doors as their own actions under a general idea of alternate actions, a game over screen with a morgue file whose contents the game drives, and a menu to restart from.
- 2026-09-16: four more items of `docs/TODO.md`: swapping on a bump, consumables through abilities, tile looks from RON, and the save seam.
  A bump into an ally was always a free refusal; `BumpRules::swap_allies` makes it a `Swap`, resolved in `ResolveSet::Travel` like a step, so a game with a companion writes nothing.
  Corsair's `use_items` hardcoded what rum does while `abilities.ron` already had a `swig` costing a rum; now the bottle `Grants` the swig and the swig costs a `Charge`, which the engine spends from the bottle, so a consumable is a line of RON and `UseItem` is an alternate action that comes to `Use`. `Known` learns from the bag as well as the slots, since a wand in a pack is a wand; an ability that should need its item worn says so with `Requirement::Wielding`.
  Every game built its `TileAppearance` in Rust; `TileAppearance::load` reads the same from RON, checked against the registry with every missing tile named, and the tutorial gains the file in chapter 8, where content moves into files, leaving chapter 1 its three lines to explain what the two colours are.
  Corsair's save was 547 lines walking its own entities by name and `SaveId`; the engine walks them now. A game implements `Saveable` on the component that marks a kind, saying how to write one down and spawn it again, and the engine captures and restores what it owns on each: place, health, bag, slots, statuses, stack, transition. Each kind's entries are the kind's own RON inside the save, through `RawValue`, so the engine never learns their shape and a kind's change is the game's version bump; the engine's `RunSave::FORMAT` is checked beside it. `SavePlugin` owns the stash refresh and deletes the slot when the run ends. Corsair's module is 395 lines, most of them tests.
  Found on the way: the first turn of a run went to whichever actor a query walked first, which a new component anywhere could reorder; new actors are admitted player first, then in spawn order.
- 2026-09-16: the minds opened, section 2 of `docs/TODO.md`, in the six stages of `docs/design/minds.md`.
  Three reviews of the first plan moved it: `Watchers` is read by the panels every frame outside the turn loop, so a per-thinker sight buffer could never serve it, and every actor carries a `Viewshed` instead (Nate: sight the same for the player and the monsters); a mind's `Attack` needs the action's sweeper, not its message, or a combat-free game freezes on a blow nobody resolves; and fields cached per pass would have flooded once per monster in the faction war they were meant for, so they are keyed by goal and kept by cost epoch.
  Found on the way: the first turn of a run went to whichever actor a query walked first; the fingerprint's entity indices moved when a system was added, since systems are entities; a blanket `Sense` impl on the box answered with the box's own type.
  Every recorded replay before this desyncs, once: the minds and stealth roll from streams of their own.
- 2026-09-17: weapon speed, and eight facings for a prefab.
  `MeleeAttack` and `RangedAttack` gain `cost`, hundredths of a step that `resolve_attacks` charges instead of the flat `BASE_ACTION_COST`; `None` charges exactly that base cost, so nothing built against the old shape moves, and a shot that finds nothing in reach still spends the ordinary turn it aimed rather than a free one.
  A game asked for a fast weapon and a slow one to actually differ, which the cost had never let them do.
  `rl-mapgen` gains `Prefab::rotated` and `Prefab::flipped`, which carry a piece's marks with its tiles rather than leaving them behind, `Orient` (`Fixed`, `Turned`, `TurnedOrMirrored`) on `StampPrefab` drawing a facing from the pass's own stream, and `StampOneOf`, one stamp choosing among several weighted candidates and failing the chain loudly when none carries weight.
  `Fixed` draws nothing from the stream, so every map already built from a `StampPrefab` is unchanged; one authored vault laid down in eight facings is most of what a hand-built dungeon means by variety.
  Found on the way: a test's comment named a theme weapon by its fantasy or sci-fi words, which the engine's own rule against theme vocabulary bars even in a test; and stamping three rooms of size 8 to 10 on a 40x30 map fails roughly one seed in sixty inside `Rooms`'s default thirty attempts, recorded in `docs/TODO.md` since it is not yet decided whether that is tuning or a limit worth documenting outright.
- 2026-09-17: noise and hearing, phases A to E of `docs/design/noise.md`, the second sense `docs/design/stealth.md` deferred.
  `rl-rules` gains `ai::hearing`: `HearingStats`, and `carries`, `left_after` and `heard`, with what sound passes read from the flags a tile already has.
  `rl-bevy` gains `noise`: `NoisePlugin` and `NoiseRules`, `MakeNoise` from the engine's own steps, blows, doors and landings and from a game, `Sounds` interned as tones are, `Hearing`, `Heard`, `NoiseHeard`, and `TurnSet::Listen` after `React`, where every noise of a pass floods once through walls and doors; the move resolver gains `Stepped`; `Thinking` takes trails by offer, so stealth and hearing each offer theirs and a search follows the freshest.
  A listener hears a place and never who made it, and goes to look unless it can already see the spot.
  The heist's hand-built earshot, a radius through walls and a pebble disguised as a hider, is gone for the engine's; the delve's rats and salt ghosts hear a fight round a corner.
...
- 2026-09-21: remains, and Foundry's design document.
  Nate wrote a game design document for Foundry, `examples/foundry/DESIGN.md`, modelled on the Bracket tutorial's chapter 44: the full ten-deck game, what exists marked against what is planned, and under each mechanic what it asks of the engine.
  Four of its mechanics, a repair drone rebuilding a droid wreck, a salvager stripping one, a scrap crab carrying it off and the commando searching it, all wanted the same thing underneath, which the engine did not have: something left where an actor fell.
  Nate, 2026-09-21: "I want the engine to not make assumptions about how corpses will be used".
  So the remains are the dead actor itself, kept past the frame it died in, with only `WasLiving` taken off; nothing is spawned and nothing is copied, and a game reads what died from the components it wrote itself.
  Two levels of opt-in, the plugin and `LeavesRemains` per actor, because whether anything stays behind is a property of a kind of thing rather than of a game.
  The one thing the engine says about a body is whose it was, in `RemainsView`, because sides are its own; anything finer travels as a game's `Sense`.
  No decay, since how long the dead linger is a rule about a world, and no engine tactic, since what a mind does on reaching a body is always the game's.
  The cost, written down in the design: a game querying its own monsters by its own component now matches its bodies too and needs `Without<Remains>`.
  Foundry's own wrecks, the searching, the stripping and the repair drones are the next slice, in the game rather than the engine.
- 2026-09-21: the inventory is enforced, and the plugin table lands.
  A review of the engine against its documentation found the coverage good and the enforcement absent: `AGENTS.md` carried "a slice that adds or removes a system updates `docs/OVERVIEW.md` in the same commit" under *rules that the build enforces*, and it was the only bullet in that section with no CI step behind it.
  The drift that had accumulated was small but real: sixteen of the fifty-one plugins were missing from the inventory, six of them described there in prose but never named by type, and ten of them, the `*ViewPlugin`s a game needs by name to write its own presenter over a view the engine already collects, named in no document at all; the overview said lighting was opt-in by inserting a `Lighting`, which is the "a resource happens to exist" pattern the rules ban and has been `LightingPlugin` for some time; the layout section listed four design docs of nine; and the overview's hand-written "Last updated" stamp was four days behind content it already described.
  `scripts/check-overview.sh` closes it, and CI runs it: every `impl Plugin for X` is named in the overview, every design doc is listed in the layout section, every game in `examples/` is described.
  Only what can be checked exactly, because a doc check with false failures is one everybody learns to ignore; whether a sentence is still true stays a review matter.
  The stamp is gone rather than checked, since a date that must be bumped by hand drifts by construction and `git log -1` already knows; and the rule is widened from one file to four, naming which of the overview, the changelog, the README's feature list and a design doc a slice owes and when.
  The plugin table `docs/TODO.md` asked for is in the overview's `rl-bevy` section, with its needs split into the plugins a plugin checks for in `finish` and the resources it declares with `needs`, since the engine itself distinguishes them; the presenter table beside it in `rl-ui` names every view and view plugin.
  The README's feature list, which had stopped at roughly the M6 engine, gains items, abilities, stealth, noise, fire and gas, props and remains, the panel architecture, and replay and the morgue.
  The subsystems still owed a design doc are written down in `AGENTS.md` rather than left to be rediscovered: the turn loop with its cues and holds, combat and `Loadout`, items and equipment, places and streaming, saving and the morgue, registries and content loading, and the controls, modals and cursors.
- 2026-09-23: effects become a subsystem, and items say what they do.
  Nate, 2026-09-23: "It doesn't make sense for us to mix item use effects with abilities. They are two different concepts sharing the same Effects code."
  Foundry's grenades were abilities in `abilities.ron` whose `Charge(1)` destroyed the item that lent them, and the effect machinery lived in `ability.rs`, so props reached into abilities to land a trap.
  `EffectsPlugin` is now the subsystem abilities, props and items use as peers, with `EffectRng` on the old stream's domain so no seed rolls differently.
  What a prop or an item does is `Triggers`: a moment from an open registry, an area, a count of fires and a list, and a trigger with no list delivers the thing's shared `effects`, which is the potion model, one bottle drunk or thrown.
  Nate on items that are aimed: "Guns are technically aimed", so an item is aimed only by being thrown or fired, each owned by the subsystem that already did it, and a wand is a weapon whose shot carries effects.
  Nate: "consumable shouldn't be a boolean, but have charges", so `Consumable` counts charges with what happens at the last and an optional refill on the clock, and `Charges`, `OnUse` and `Cost::Charge` are gone with item `Grants`.
  A subsystem that owns a moment writes `Fired` and nothing else, and `land_triggers` lands it in a new `ResolveSet::Triggers` between the actions and the fields, so a grenade's fire spreads and a stim mends in the pass that set them off; a trap now lands a pass sooner and a `destroyed` trigger a pass later.
  Found on the way: Foundry's `t` opened the pack and the pack threw in the same keypress, so a screen no longer reads keys in the frame it opened; and the cheat menu's added item was logged twice.
