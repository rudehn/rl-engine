...
- The narrator: `NarrationView`, rows of `Said` (a `Phrase`, who, whom, what, an amount, a registry name, where, and whether the player saw it), collected in `TurnSet::React` one pass at a time so a frame of many turns reads in the order they happened; and `NarratorPlugin`, which speaks each row in `ViewSet::Speak` through a `Phrasebook` of one template and tone per phrase, split by perspective (`YouHit`, `HitsYou`, `OthersFight`, `YouKill`, `Dies`, `YouPickUp`, `Wields`, `OpensDoor`, `NoticesYou`, `YouUseOn`, `YouAreAfflicted`, `YourLightGoesOut`, and the rest), with names coloured as the things they name. A game rewords or silences any phrase, speaks the unseen or not, edits rows in `ViewSet::Annotate`, or reads the view itself.
...
- Panels: `NearbyPanel`, `VitalsPanel`, `GearPanel`, `InspectPanel` (the cursor as four pulsing pointers on the cells around the one looked at, which keeps its own glyph), `LogPanel`, `ScrollbackPanel`, `TargetPanel`, `AbilityPanel`, `InventoryPanel`, `ControlsPanel` and `SheetPanel`, each a plugin holding its rectangle and its headings.
...
Its sea chest is the engine's `InventoryPanel` under another name, and a bottle of rum is described by the swig it grants.
...
Its abilities are a pirate's: a broadside on powder, a grapnel, a swig of rum that the bottle lends and costs, and a shakedown, `Plunder`, which is Corsair's own effect; cutthroats throw the grapnel back, and its cave dwellers notice rather than see on sight.
