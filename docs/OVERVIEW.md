...
- Cues: what a turn did that is worth seeing, as `Cued` messages the resolvers write, each a `Cue::Flight` between two `Anchor`s or a `Cue::Burst` on some, in a `LookOf` the drawer resolves. An anchor is a cell or an entity on one, followed as it moves. `TurnHold`: a plugin that plays cues `watch`es, and the loop then stops after any pass that cued something for anyone but the player and runs no pass until the plugin lets go, so two casters are seen one after the other and a cue that lands on the player never keeps them waiting. A flight lands when it has been seen: with something watching, the ability resolver and the throw resolver cue the flight, put the landing aside in `Airborne` and `AirborneThrows`, and spend the turn; no turn is dealt while anything is in the air; the first pass after the hold lets go lands it, which is when the burst is cued, the effects run, the blow falls and the knife lies where it fell; and the burst is waited on in turn. Without a watcher a use lands as it is cast. An effect writes its own cues through `EffectWorld::cues`.
