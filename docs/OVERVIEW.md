...
- Particles: `Particles` plays `Animation`s of `Trail` (a glyph along a path with a fading tail) and `Burst` (a footprint lit at once and fading, each cell one of a few glyphs) steps from the wall clock over the map, never waiting for. `ParticlesPlugin`, in `RoguelikePlugins`, plays a flight and a burst for every ability used in the ability's `look` (or `ParticleStyle`'s plain one) and a flight for every item thrown in its glyph; an animation is left behind on the map it played on.
