...
- The targeting cursor: a game writes `AimAt` for an ability or `AimThrow` for something carried, and the engine does the rest. A throw opens on the nearest foe, previews through `flight` who it strikes first and where it lands, and writes the `Throw` on confirm; the banner says what is aimed by the name the view carries.
