...
- Panels: `NearbyPanel`, `VitalsPanel`, `GearPanel`, `InspectPanel` (the cursor as four pulsing pointers on the cells around the one looked at, which keeps its own glyph), `LogPanel`, `ScrollbackPanel`, `TargetPanel`, `AbilityPanel`, `ControlsPanel` and `SheetPanel`, each a plugin holding its rectangle and its headings.
