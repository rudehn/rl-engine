...
It chains `follow_player`, `track_fields` and `draw_map` in `PresentSet::Map`, and puts `dress_props` before `draw_map` in plain `Update` rather than in a play-only set, because a prop is put down while a place is built and `Added` matches for one frame only.
...
`FieldAppearance` is fire and gas over the tiles they are on: a `flame` cell and the `flare` it flickers toward, the `ember` that now and then rises off a flame into the cell over it, the `haze` glyphs gas thick enough to hide behind is drawn with, thinnest first, and a tint per `GasId` that is grey until the game names one.
What the map draws of fire and gas is `ShownFields`, not the fields as they stand: when a cell's fields change it blends from what it showed to what they hold over `BLEND_SECS`, so a cloud rolls on a turn rather than jumping, and a player who acts mid-blend sees it pick up from where it had got to.
A blend starts when the last `Wavefront` still playing reaches the cell, which is how the smoke and fire a blast left show behind its front rather than before it has gone off.
`churn` stirs how thick a cloud looks, in patches that swell and thin with the wall clock, so smoke hanging between turns still moves, and a cloud in sight is drawn whole over the ground the player knows through `ShownFields::clouds_in_sight`, since a cloud that hides its own far side would otherwise read as a wall.
...
A `Beat` is a `Trail`, a glyph flying between two anchors with three cells of fading tail, or a `Burst`, every anchor lit and fading, each cell showing one of a few glyphs picked by position so it reads as embers rather than a stamp.
A `Burst` with a `from` is a blast: each anchor lights `ring_secs` later for every cell it is further from the source, give or take half a ring so the front is ragged, burns down its glyphs from white-hot to embers while lighting the ground under it, and throws a few sparks ahead of the front to its far edge.
`Beat::frame` answers with `Spark`s, a cell, a glyph, a colour, how far it has faded and how far it lights the ground, and it is handed a resolver for anchors, so the line is redrawn every frame between where the two anchors are now and a flight at someone who walks on bends to follow them.
`ParticleStyle` is the pace and the look of anything with none of its own: the `plain` glyph and colour, seconds per cell of flight, seconds a burst takes, the glyphs a burst picks from, and a blast's `ring_secs` and the `blast_glyphs` it cools through.
...
`play_cues` turns the frame's `Cued` messages into one animation per actor, that actor's cues in the order they were written, each holding the turns, and `publish_wavefronts` tells the map where every blast's front is before the map is drawn.
...
Fire and gas are drawn on tiles in sight and on the rest of a cloud in sight over known ground, and nowhere else, because memory holds no smoke and a cloud must not show the shape of a room nobody has seen; what hides behind a haze is decided by the map's opacity rather than by the renderer's taste.
The blending and the waiting are the drawing's alone: what the fields hold, what hides whom and what a turn does never wait for a blast to reach a cell, so a headless game plays the same turns.
...
The particles' arithmetic is `Beat::frame` and `Animation::frame` over a time and a resolver, which is why what a flight shows at a given moment is a test rather than a screenshot, and `fields.rs` is the same for the map's side of a blast: when a cell's smoke shows and how thick is `ShownFields` over a time, with no `App`.
