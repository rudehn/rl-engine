...
It adds `offer_here` in `DecideSet::Offer`, beside the ability gate; `resolve_interactions` and `resolve_takes` in `ResolveSet::Act`; `spot_hidden_props` in `DecideSet::Notice`, beside the stealth roll it resembles; `perceive_props` in `PerceiveSet::Annotate`; and `close_emptied_containers`, `spring_on_entered` and `spring_on_destroyed` in `TurnSet::React`.
Outside the turn loop it configures `PropSet::Stock` then `PropSet::Fill` inside `EngineSet::Stream`, because a prop is put down while a place is built: the engine builds every prop's effects and asks for what each container holds in the first, and a game answers in the second, so what goes into a crate lands in the frame the crate was put down rather than in that frame or the next depending on which way the executor ran two systems.
...
It takes two streams, `PropRng` for what a container holds and what is spotted, and abilities' own `AbilityRng` for what a trigger's effects roll, because a trap lands the same effects an ability does and wants the same dice.
...
`PropDef` is what a `props.ron` says: a `name`, a `Look` of glyph, colour and layer, `blocks`, an optional `health`, a list of `OfferDef`, and optional `container`, `trigger` and `hidden`.
...
Both land through `Landing`, whose `ability` is `None` for a trap, and both write `Triggered { prop, on, by, at }` for what no effect can say.
...
`rl-bevy` is tier 2 and owns everything that happens: `props.rs` is the plugin, the components, the gate, the two actions, the triggers and the spotting roll, and it is one file because a prop's parts are one idea seen from several sides.
