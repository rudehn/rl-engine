...
- **A subsystem is a plugin, and a plugin is opt-in.** Whether a system runs is decided by whether its plugin was added, never by whether a resource happens to be there. A plugin declares what it cannot work without via `needs`, and says so loudly at startup when it is missing.
...
docs/design/               how one subsystem works, and why: lighting, ui
